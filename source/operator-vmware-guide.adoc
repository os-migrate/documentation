...
[id="custom-nbdkit-plugin"]
=== Using a Custom nbdkit Plugin

By default, OS Migrate uses the nbdkit `vddk` plugin to transfer VMware virtual machine disks. In some scenarios, you may want to bypass VDDK and use an alternative nbdkit plugin (for example, a third-party plugin that implements the NBD or NFC protocol).

The `custom_nbdkit_plugin` variable allows you to specify the absolute path to a custom nbdkit plugin shared library (`.so` file) on the conversion host.
...
* VDDK-specific parameters (`libdir`, `compression`, `transports`, disk name) are omitted from the nbdkit command
* Only the common parameters (`server`, `user`, `password`, `thumbprint`, `vm`, `snapshot`) are passed
...
* The plugin must be compatible with nbdkit and accept the parameters passed by OS Migrate
* Leave `custom_nbdkit_plugin` empty or unset to use the default VDDK plugin

NOTE: When using a custom plugin, ensure it supports the necessary VMware connection parameters and snapshot handling required for migration.
