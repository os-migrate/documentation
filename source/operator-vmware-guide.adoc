...
Verify that the snapshot hierarchy is not too deep (recommended maximum: 2-3 levels).

Deep snapshot hierarchies can cause:

* Performance degradation during migration
* Increased risk of migration failures
* Longer consolidation times

To check snapshot hierarchy:
...
Ensure the VMware user account has the correct permissions to perform migration operations.
...
For CBT-based migrations, additional privileges may be required:

** Virtual machine.Configuration.Change tracking
...
[source,powershell]
----
Get-VM <vm-name> | Select Name, @{N='CBT';E={$_.ExtensionData.Config.ChangeTrackingEnabled}}
----
